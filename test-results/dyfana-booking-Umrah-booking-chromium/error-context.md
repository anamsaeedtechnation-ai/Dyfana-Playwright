...
- Name: dyfana-booking.spec.ts >> Umrah booking
- Location: tests\dyfana-booking.spec.ts:26:5
...
Call log:
  - waiting for getByRole('link', { name: 'Umrah Umrah' })
...
> 27  |   await page.getByRole('link', { name: 'Umrah Umrah' }).click();
  ...
  122 | });
  123 | 
  124 | test('Hotel booking', async ({ page }) => {
  125 |   // 2-day buffer so the check-in is never in the past (timezone-safe via local date parts)
  126 |   const fmt = (d: Date) => `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
  127 |   const checkIn = fmt(new Date(Date.now() + 86400000 * 2));
```